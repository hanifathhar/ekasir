...
class piutang{
    ...
    function view($tgl1,$tgl2){
			...
			echo "<div class=\"page-header\">
						<h4 class=\"page-title\">Piutang</h4>
								...
								<div class=\"card-header\" align=\"right\">
									...
									<tr>
										<td>
											...
											<button class=\"btn btn-primary btn-sm\"><i class=\"fa flaticon-pen\"></i> Pelunasan Piutang</button>
											</a>
										</td>
										<td>
											&nbsp;
										...
										<table id=\"basic-datatables\" class=\"display table table-striped table-hover\" >
											<thead>
												  <tr>
													...
													<th class=\"text-center\" style=\"background-color:#000033; color:#FFFFFF; font-size:12px;\" width=100>Nilai Piutang</th>	
													<th class=\"text-center\" style=\"background-color:#000033; color:#FFFFFF; font-size:12px;\" width=100>Nilai Pelunasan</th>	
													<th class=\"text-center\" style=\"background-color:#000033; color:#FFFFFF; font-size:12px;\" width=100>Sisa Piutang</th>												
											...
											<tbody>";
											 ...
											 $baca = mysqli_query($conn,"SELECT  *,DATE_FORMAT(tgl_transaksi, '%d/%m/%Y') AS tgl,IF(nm_member<>'',nm_member,'Umum') as pembeli,
											         (select nm_pengguna from admin where id=tbl_piutang.user) as kasir,DATE_FORMAT(tgl_faktur, '%d/%m/%Y') AS tgl_faktur
											         FROM tbl_piutang WHERE tgl_transaksi BETWEEN '$tgl1' and '$tgl2' AND lunas='1' ORDER BY no_transaksi,tgl_transaksi");
											 ...
											 echo "<tr>
													...
													<td class=\"text-right\" style=\"font-size:12px;\">".number_format($fetchArray['total_piutang'])."</td>
														...
														<input type=\"hidden\" value=\"$fetchArray[no_transaksi]\" name=\"idx\">
														  <div class=\"modal-content\">
															<div class=\"modal-header\" style=\"background-color:#000033; color:#FFFFFF;\">
															  <h5 class=\"modal-title\" id=\"exampleModalLabelLogout\"><i class=\"flaticon-agenda-1\"></i> Pelunasan Piutang</h5>
															</div>
														   ...
														   echo "<div class=\"form-group row\">
																  <label for=\"inputEmail3\" class=\"col-sm-3 col-form-label\"><b>Nilai Piutang</b></label>
																  <div class=\"col-sm-8\">
																	<div class=\"input-icon\">
																	<span class=\"input-icon-addon\">Rp</span>
																		<input type=\"text\" class=\"form-control\" id=\"\" style=\"text-align:right;\" name=\"\" 
																		value=\"$fetchArray[total_piutang]\" readonly>
																		</div>
																  </div>
															  </div>";
											...
											echo "</div>
											  ...
											  $total = $total+$fetchArray['total_piutang'];
														  ...
														  <div class=\"modal-content\">
														   ...
														   echo "<div class=\"form-group row\">
																  ...
																  <div class=\"col-sm-8\">
																	<div class=\"input-icon\">
																	<span class=\"input-icon-addon\">Rp</span>
																		<input type=\"text\" class=\"form-control\" id=\"total_piutang\" style=\"text-align:right;\" name=\"total_piutang\" value=\"$fetchArray[total_piutang]\" readonly>
																		</div>
																  </div>
															  </div>";
				...
				echo "<div class=\"modal fade\" id=\"daftar\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"exampleModalLabelLogout\"
														...
														<input type=\"hidden\" value=\"$kode\" name=\"id\">
														  <div class=\"modal-content\">
															<div class=\"modal-header\" style=\"background-color:#000033; color:#FFFFFF;\">
															  <h5 class=\"modal-title\" id=\"exampleModalLabelLogout\"><i class=\"fas fa-book-open\"></i> Daftar Piutang</h5>
															</div>
															<div class=\"modal-body\">";
																...
																<table id=\"basic-datatables_new2\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
																	<thead>
																		 <tr>
																			...
																			<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=500>Daftar Piutang</th>
																			<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=>Aksi</th>
																		  </tr>
																	</thead>
																	<tbody>";
																	
																	$baca = mysqli_query($conn,"SELECT * FROM tbl_piutang WHERE lunas='0' ORDER BY faktur");
																	 $no = 1;	
																	 while($fetchArray = mysqli_fetch_array($baca)){
																
																	  echo "<tr class=\"piutang\" data-a=\"$fetchArray[faktur]\" data-b=\"$fetchArray[nm_member]\" data-c=\"$fetchArray[total_piutang]\" >
																				<td style=\"\">$no</td>
																				<td style=\"\">
																				<div class=\"card-list\">
																				<div class=\"item-list\">
																					...
																					<div class=\"status\">Rp.".number_format($fetchArray['total_piutang'])."
																						<div class=\"status\">$fetchArray[keterangan]</div>
																					</div>
																				</div>
																				</div>	
																				</td>
																				<td style=\"\"><button type=\"button\" name=\"piutang\" value=\"piutang\" class=\"btn btn-icon btn-round btn-primary btn-sm\">
		  ...
		  if($_POST['simpan']){
				...
				$total_piutang = htmlspecialchars($_POST['total_piutang'], ENT_QUOTES);
				$pelunasan = htmlspecialchars($_POST['pelunasan'], ENT_QUOTES);
				$saldo = $total_piutang-$pelunasan;
				$user = $_SESSION['id'];
				
				if($saldo == 0){
					$query = mysqli_query($conn,"UPDATE tbl_piutang SET tgl_transaksi = '$tgl',
													   pelunasan = '$pelunasan',
													   saldo = '$saldo',
													   tgl_update = NOW(),
													   user = '$user',
		  ...
		  if($_POST['hapus']){
				...
				$query = mysqli_query($conn,"UPDATE tbl_piutang SET tgl_transaksi = NULL,
													   pelunasan = '0',
													   saldo = total_piutang,
													   tgl_update = NOW(),
													   user = '$user',
...
<script>
 $(document).on('click', '.piutang', function (e) {
				...
				document.getElementById("total_piutang").value = $(this).attr('data-c');
