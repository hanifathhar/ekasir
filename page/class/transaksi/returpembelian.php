...
class returpembelian{
    ...
    function view($tgl1,$tgl2){
							...
							<form action=\"\" method=\"post\">
								...
								<div class=\"card-header\">
									...
									<div class=\"form-group row\">
									  <label for=\"inputEmail3\" class=\"col-sm-2 col-form-label\" style=\"text-align:right\">Periode Retur Pembelian</label>
									  <div class=\"col-sm-2\">
								...
								<div class=\"card-header\" align=\"right\">
									...
									<tr>
										<td>
											<a href=\"welcome.php?modul=returpembelian&aksi=input\">
											<button class=\"btn btn-primary btn-sm\"><i class=\"fa flaticon-pen\"></i> Input Retur Pembelian</button>
											</a>
										</td>
										<td>
											<form method=\"post\" action=\"welcome.php?modul=returpembelian&aksi=report\" target=\"\">
										...
										<table id=\"basic-datatables\" class=\"display table table-striped table-hover\" >
											<thead>
												  <tr>
													...
													<th class=\"text-center\" style=\"background-color:#000033; color:#FFFFFF; font-size:12px;\" width=>Total<br>Pengembalian</th>
											...
											<tbody>";
											 $baca = mysqli_query($conn,"SELECT  *,DATE_FORMAT(tgl_transaksi, '%d/%m/%Y') AS tgl,IF(pembayaran=1,'Tunai','Kredit') as bayar,
											         (select nm_pengguna from admin where id=tbl_retur_pembelian.user) as kasir,IF(nm_supplier<>'',nm_supplier,'Umum') as supplier 
											         FROM tbl_retur_pembelian WHERE tgl_transaksi BETWEEN '$tgl1' and '$tgl2' ORDER BY no_transaksi,tgl_transaksi");
											 $no = 1;	
											 ...
											 echo "<tr>
													...
													<div class=\"card-list\">
														<div class=\"item-list\">
															<div class=\"info-user ml-3\">
																<div class=\"status\"><b>".($fetchArray['supplier'])."</b></div>
													...
													<div class=\"card-list\">
														<div class=\"item-list\">
															<form method=\"post\" action=\"welcome.php?modul=returpembelian&aksi=invoice\">
															<input type=\"hidden\" name=\"tgl1\" value=\"$tgl1\">
															<input type=\"hidden\" name=\"tgl2\" value=\"$tgl2\">
															...
															&nbsp;";
															$sql = mysqli_query($conn,"SELECT * from tbl_piutang where faktur='$id' and lunas='1'");
															$res = mysqli_num_rows($sql);
															if($res > 0 ){
															echo "<form method=\"post\" action=\"welcome.php?modul=returpembelian&aksi=edit\">
															<input type=\"hidden\" name=\"tgl1\" value=\"$tgl1\">
															<input type=\"hidden\" name=\"tgl2\" value=\"$tgl2\">
															...
															echo "
															&nbsp;
															<form method=\"post\" action=\"welcome.php?modul=returpembelian&aksi=hapus\">
															<input type=\"hidden\" name=\"tgl1\" value=\"$tgl1\">
														    <input type=\"hidden\" name=\"tgl2\" value=\"$tgl2\">
		  ...
		  if($_POST['tampil']){
				...
				$tgl2 = htmlspecialchars($_POST['tgl2'], ENT_QUOTES);
		  		...
		  		echo "<form action=\"welcome.php?modul=returpembelian&aksi=view\" method=\"post\" id=\"success\">
	...
	function hapus($id,$tgl1,$tgl2){
    		...
    		$query = mysqli_query($conn,"delete from tbl_retur_pembelian where no_transaksi='$id'");
			$query = mysqli_query($conn,"delete from tbl_retur_pembelian_det where no_transaksi='$id'");
			$query = mysqli_query($conn,"delete from tbl_piutang where faktur='$id'");
			...
			if($query){
				...
				echo "<script type=\"text/javascript\">
						swal(\"Maaf! ", "Data Gagal di Hapus, Transaksi telah tercatat di daftar Pelunasan Piutang\", {
						icon : \"error\",
						buttons: {        			
							confirm: {
