...
$sql = mysqli_query($conn,"SELECT  * from tbl_pembelian where no_transaksi='$id'");
$tampil = mysqli_fetch_array($sql);
$kode = $tampil['no_transaksi'];

}else{

$sql = mysqli_query($conn,"SELECT  IFNULL(MAX(SUBSTR(no_transaksi,16,6)),0) AS nom from tbl_pembelian where user='$user'");
$tampil = mysqli_fetch_array($sql);
...
?>
            ...
            <div class="card mb-12">
					...
					<div class="col-md-12 col-lg-7"> 
						...
						echo "<div class=\"table-responsive\">
										<table id=\"basic-datatables\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
											...
											<tbody>";
											$baca = mysqli_query($conn,"SELECT  * FROM tbl_pembelian_det where no_transaksi='$kode' ORDER BY id");
											$no = 1;	
				...
				<div class="card-action" align="right">
					...
					if($res < 1 ){
						...
						echo "<a href=\"welcome.php?modul=pembelian&aksi=view\"><button type=\"button\" class=\"btn btn-danger\"><i class=\"fas fa-reply\"></i> Kembali</button></a>";
					}else{
              ...
              <div class="modal-content">
				...
				echo "<div class=\"table-responsive\">
										<table id=\"basic-datatables_new\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
											...
											$baca = mysqli_query($conn,"SELECT * FROM tbl_barang ORDER BY kd_barang");
											 $no = 1;	
											 while($fetchArray = mysqli_fetch_array($baca)){
										      ...
										      echo "<tr class=\"barang\" data-a=\"$fetchArray[kd_barang]\" data-b=\"$fetchArray[nm_barang]\"
														...
														data-e=\"$fetchArray[harga_beli]\"
														data-f=\"".number_format($fetchArray['harga_beli'])."\">
		  ...
		  <div class="modal fade" id="formbayar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelLogout"
			...
			<input type="hidden" value="<?php echo $kode;?>" name="no_transaksi">
			<input type="hidden" value="<?php echo "$aksi";?>" name="aksi">
              <div class="modal-content">
                <div class="modal-header" style="background-color:#003399; color:#FFFFFF;">
                  <h5 class="modal-title" id="exampleModalLabelLogout"><i class="fas fa-check"></i> Pembelian :</h5>
                </div>
                <div class="modal-body">
				<div class="row">
                <div class="col-md-12 col-lg-6" > 
					<div class="form-group row">
					  ...
					  <div class="form-group row">
						  ...
						  <div class="col-sm-8">
							<input type="text" class="form-control" id="faktur" style="text-align:left;" name="faktur" value="<?php echo $kode;?>" required >
						  </div>
					  </div>
					  <div class="form-group row">
		  ...
		  //jika dipilih, nim akan masuk ke input dan modal di tutup
			...
			function tampilkanNilai() {
			  ...
			  hasil.innerHTML = "Anda memilih: " + selectedValue;
			}
...
$total = $harga*$jumlah;
		...
		if(empty($kd_barang) || empty($nm_barang) || empty($kd_kategori) || empty($satuan) || empty($harga) || empty($jumlah)){
				...
				echo "<form action=\"welcome.php?modul=pembelian&aksi=$aksi\" method=\"post\" id=\"gagal\">
				...
				$query = mysqli_query($conn,"INSERT INTO tbl_pembelian_det (no_transaksi,tgl_transaksi,kd_barang,nm_barang,kd_kategori,satuan,harga,jumlah,total,tgl_update,user) 
									  VALUE('$id','$tgl','$kd_barang','$nm_barang','$kd_kategori','$satuan','$harga','$jumlah','$total',NOW(),'$user')");
					
				if($query){
				
				echo "<form action=\"welcome.php?modul=pembelian&aksi=$aksi\" method=\"post\" id=\"success\">
						  <input type=\"hidden\" value=\"$id\" name=\"id\">
					</form>";
...
if($_POST['hapus']){
		...
		$query = mysqli_query($conn,"delete from tbl_pembelian_det where id='$id'");
...
$saldo = $grandtotal-$dibayar;
				...
				$query = mysqli_query($conn,"delete from tbl_pembelian where no_transaksi='$no_transaksi'");
				$query = mysqli_query($conn,"INSERT INTO tbl_pembelian (no_transaksi,tgl_transaksi,id_supplier,nm_supplier,faktur,pembayaran,exspedisi,ongkir,total,voucher,diskon,pajak,grand_total,dibayar,saldo,tgl_update,user) 
				...
				$query = mysqli_query($conn,"update tbl_pembelian_det set tgl_transaksi='$tgl' where no_transaksi='$no_transaksi'");
				...
				if($pembayaran=='2'){
					...
					$query = mysqli_query($conn,"INSERT INTO tbl_hutang (no_transaksi,tgl_faktur,id_member,nm_member,faktur,total_hutang,pelunasan,saldo,tgl_update,user,keterangan) 
									 VALUE('$kode','$tgl','$id_supplier','$nm_supplier','$faktur','$grandtotal','','$saldo',NOW(),'$user','Pembelian')");
				
				}
					
				if($query){
					
					echo "<form action=\"welcome.php?modul=pembelian&aksi=invoice\" method=\"post\" id=\"success\">
					          <input type=\"hidden\" value=\"$faktur\" name=\"id\">
						  </form>";	
