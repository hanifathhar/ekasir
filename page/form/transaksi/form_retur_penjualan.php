...
$sql = mysqli_query($conn,"SELECT  * from tbl_retur_penjualan where no_transaksi='$id'");
$tampil = mysqli_fetch_array($sql);
$kode = $tampil['no_transaksi'];

}else{

$sql = mysqli_query($conn,"SELECT  IFNULL(MAX(SUBSTR(no_transaksi,16,6)),0) AS nom from tbl_retur_penjualan where user='$user'");
$tampil = mysqli_fetch_array($sql);
...
?>
            ...
            <div class="card mb-12">
					...
					<div style="border:1.7px solid #aaa;border-radius:8px;padding:12px; background-color:#FFFF99;no-repeat center center fixed;
						...
						echo "<div class=\"table-responsive\">
										<table id=\"basic-datatables\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
											...
											<tbody>";
											$baca = mysqli_query($conn,"SELECT  * FROM tbl_retur_penjualan_det where no_transaksi='$kode' ORDER BY id");
											$no = 1;	
				...
				<div class="card-action" align="right">
					...
					if($res < 1 ){
						...
						echo "<a href=\"welcome.php?modul=returpenjualan&aksi=view\"><button type=\"button\" class=\"btn btn-danger\"><i class=\"fas fa-reply\"></i> Kembali</button></a>";
					}else{
										...
										<table id=\"basic-datatables_new\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
											<thead>
												 <tr>
													...
													<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=50>Terjual</th>
													<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=10>Aksi</th>
												  </tr>
											</thead>
											<tbody>";
											
											$baca = mysqli_query($conn,"SELECT *,IFNULL((terjual),0) as saldo FROM tbl_barang ORDER BY kd_barang");
											 $no = 1;	
											 while($fetchArray = mysqli_fetch_array($baca)){
											  ...
											  if($stock < 1){												
												  echo "<tr class=\"\" data-a=\"$fetchArray[kd_barang]\" data-b=\"$fetchArray[nm_barang]\"
															...
															data-h=\"$fetchArray[harga_beli]\" style=\"color:#FF0000;\">
		  ...
		  <div class="modal fade" id="formbayar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelLogout"
			...
			<input type="hidden" value="<?php echo "$aksi";?>" name="aksi">
              <div class="modal-content">
                <div class="modal-header" style="background-color:#003399; color:#FFFFFF;">
                  <h5 class="modal-title" id="exampleModalLabelLogout"><i class="fas fa-check"></i> Konfirmasi Retur Penjualan :</h5>
                </div>
                <div class="modal-body">
				<div class="row">
                <div class="col-md-12 col-lg-6" > 
					<div class="form-group row">
					  ...
					  <div class="form-group row">
						  <label for="inputEmail3" class="col-sm-4 col-form-label">Faktur Penjualan</label>
						  <div class="col-sm-8">
							<div class="input-icon">
								<input type="text" class="form-control" placeholder="" value="<?php echo " $data[faktur]";?>" id="faktur" name="faktur">
									<span class="input-icon-addon">
									 <button data-toggle="modal" data-target="#formpenjualan" type="button" class="btn btn-default btn-sm"><i class="fa fa-search"></i> cari</button>
									</span>
								</div>
						  </div>
					  </div>
		  ...
		  <div class="modal fade" id="formmember" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelLogout"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
			<form action="" method="post">
              <div class="modal-content">
                <div class="modal-header" style="background-color:#000033; color:#FFFFFF;">
                  <h5 class="modal-title" id="exampleModalLabelLogout"><i class="fas flaticon-folder"></i> Member :</h5>
                </div>
                <div class="modal-body">
                <?php 
										...
										<table id=\"basic-datatables_new2\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
											<thead>
												 <tr>
													...
													<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=500>Nama Member</th>
												  </tr>
											</thead>
											<tbody>";
		  ...
		  <div class="modal fade" id="formpenjualan" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabelLogout"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
			<form action="" method="post">
              <div class="modal-content">
                <div class="modal-header" style="background-color:#000033; color:#FFFFFF;">
                  <h5 class="modal-title" id="exampleModalLabelLogout"><i class="fas flaticon-folder"></i> Faktur Penjualan :</h5>
                </div>
                <div class="modal-body">
                <?php 
				...
				echo "<div class=\"table-responsive\">
										<table id=\"basic-datatables_new3\" class=\"display table table-striped table-hover\" style=\"border:1.7px solid #aaa;border-radius:0px;padding:18px;\">
											<thead>
												 <tr>
												 	<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=5>No.</th>
													<th class=\"text-center\" style=\"background-color:#003399; color:#FFFFFF\" width=500>Faktu Penjualan</th>
												  </tr>
											</thead>
											<tbody>";
											
											$baca = mysqli_query($conn,"SELECT *,IF(nm_member<>'',nm_member,'Umum') as pembeli FROM tbl_pembayaran ORDER BY no_transaksi");
											 $no = 1;	
											 while($fetchArray = mysqli_fetch_array($baca)){
										
										      echo "<tr class=\"faktur\" data-a=\"$fetchArray[no_transaksi]\" data-b=\"$fetchArray[id_member]\" data-c=\"$fetchArray[nm_member]\">
														<td style=\"\">$no</td>
														<td style=\"\">
														<div class=\"card-list\">
														<div class=\"item-list\">
		  ...
		  //jika dipilih, nim akan masuk ke input dan modal di tutup
			...
			$(document).on('click', '.faktur', function (e) {
				...
				document.getElementById("id_member").value = $(this).attr('data-b');
				document.getElementById("nm_member").value = $(this).attr('data-c');
				
                $('#formpenjualan').modal('hide');
...
$total = $harga*$jumlah;
		...
		if(empty($kd_barang) || empty($nm_barang) || empty($kd_kategori) || empty($satuan) || empty($harga) || empty($jumlah)){
				...
				echo "<form action=\"welcome.php?modul=returpenjualan&aksi=$aksi\" method=\"post\" id=\"gagal\">
				...
				$query = mysqli_query($conn,"INSERT INTO tbl_retur_penjualan_det (no_transaksi,tgl_transaksi,kd_barang,nm_barang,kd_kategori,satuan,hpp,harga,jumlah,total,tgl_update,user) 
									  VALUE('$id','$tgl','$kd_barang','$nm_barang','$kd_kategori','$satuan','$hpp','$harga','$jumlah','$total',NOW(),'$user')");
					
				if($query){
				
				echo "<form action=\"welcome.php?modul=returpenjualan&aksi=$aksi\" method=\"post\" id=\"success\">
						  <input type=\"hidden\" value=\"$id\" name=\"id\">
					</form>";
...
if($_POST['hapus']){
		...
		$query = mysqli_query($conn,"delete from tbl_retur_penjualan_det where id='$id'");
...
$saldo = $grandtotal-$dibayar;
				...
				$query = mysqli_query($conn,"delete from tbl_retur_penjualan where no_transaksi='$no_transaksi'");
				$query = mysqli_query($conn,"INSERT INTO tbl_retur_penjualan (no_transaksi,tgl_transaksi,id_member,nm_member,faktur,pembayaran,exspedisi,ongkir,total_hpp,total,voucher,diskon,pajak,grand_total,dibayar,saldo,tgl_update,user) 
									  VALUE('$no_transaksi','$tgl','$id_member','$nm_member','$faktur','$pembayaran','$exspedisi','$ogkir','$total_hpp','$total','$voucher','$diskon','$pajak','$grandtotal','$dibayar','$saldo',NOW(),'$user')");
									  
				$query = mysqli_query($conn,"update tbl_retur_penjualan_det set tgl_transaksi='$tgl' where no_transaksi='$no_transaksi'");
				
				$kode = "HUT/".$faktur;
				$query = mysqli_query($conn,"delete from tbl_hutang where no_transaksi='$kode'");
				if($pembayaran=='2'){
					
					$query = mysqli_query($conn,"INSERT INTO tbl_hutang (no_transaksi,tgl_faktur,id_member,nm_member,faktur,total_hutang,pelunasan,saldo,tgl_update,user,keterangan) 
									 VALUE('$kode','$tgl','$id_member','$nm_member','$faktur','$grandtotal','','$saldo',NOW(),'$user','Retur Penjualan')");
				
				}	
				if($query){
					
					echo "<form action=\"welcome.php?modul=returpenjualan&aksi=view\" method=\"post\" id=\"success\">
