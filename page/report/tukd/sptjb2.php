...
$ttw1 = mysql_query("SELECT  sum(nilai) as nilai FROM trd_fktp WHERE kd_skpd='$kode' and MONTH(tgl_fktp) IN ('1','2','3')");
$ptw1 = mysql_fetch_array($ttw1);
$htw1 = number_format($ptw1['nilai']);
$ttw2 = mysql_query("SELECT  sum(nilai) as nilai FROM trd_fktp WHERE kd_skpd='$kode' and MONTH(tgl_fktp) IN ('4','5','6')");
$ptw2 = mysql_fetch_array($ttw2);
$htw2 = number_format($ptw2['nilai']);
$ttw3 = mysql_query("SELECT  sum(nilai) as nilai FROM trd_fktp WHERE kd_skpd='$kode' and MONTH(tgl_fktp) IN ('7','8','9')");
$ptw3 = mysql_fetch_array($ttw3);
$htw3 = number_format($ptw3['nilai']);
$ttw4 = mysql_query("SELECT  sum(nilai) as nilai FROM trd_fktp WHERE kd_skpd='$kode' and MONTH(tgl_fktp) IN ('10','11','12')");
...
$bepega2 = mysql_query ("SELECT SUM(a.nilai) AS bpeg FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$kode' and a.kd_rek5 LIKE '521%'");  	
...
$bbaja2 = mysql_query("SELECT IFNULL(SUM(a.nilai),0) AS bbj FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$kode' and LEFT(a.kd_rek5,3)='522'");  	
...
$bmodal2 = mysql_query ("SELECT IFNULL(SUM(a.nilai),0) AS bmod FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$kode' and a.kd_rek5 LIKE '523%'");	
...
$sisasmt2 = number_format(($ptw1['nilai'] + $ptw2['nilai'] + $ptw3['nilai'] + $ptw4['nilai']) - ($bepeg2['bpeg'] + $bbja2['bbj'] + $bmoda2['bmod']));
		...
		echo "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\"> 
					...
					<tr>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>&nbsp;&nbsp;&nbsp;1. Triwulan I</td>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>Rp. $htw1";
						echo "</td>
					</tr>
					<tr>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>&nbsp;&nbsp;&nbsp;2. Triwulan II</td>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>Rp. $htw2</td>
					</tr>
					<tr>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=3 align=\"left\"><b>Jumlah Semester I</b></td>
						<td width=\"25%\"  style=\"font-size:12px;width:10%;border-bottom: 2px solid\"><b>Rp. $totalsmt1</b></td>
					</tr>
					<tr>
					...
					<tr>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>&nbsp;&nbsp;&nbsp;3. Triwulan III</td>
					...
					<tr>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>&nbsp;&nbsp;&nbsp;4. Triwulan IV</td>
						<td width=\"25%\"  style=\"font-size:12px;border:solid 0px black;\" colspan=2>Rp. ";
						if ($data[bulan] == '1'){
							echo '0';
						} else if ($data[bulan]=='2'){
					...
					<tr>
						...
						if ($data[bulan] == '1'){
							...
							echo number_format($saldo + $ptw1['nilai'] + $ptw2['nilai'] + $ptw3['nilai'] + $ptw4['nilai']);
						}
						echo "</b></td>
					</tr>
					...
					<tr>
						...
						<td width=\"25%\"  style=\"font-size:12px;width:10%;border-bottom: 2px solid\"><b>Rp. ";
						if ($data[bulan] == '1'){
							echo number_format(($saldo + $ptw1['nilai'] + $ptw2['nilai']) - ($bepeg ['bpeg'] + $bbja['bbj'] + $bmoda['bmod']));
							$kas = ($saldo + $ptw1['nilai'] + $ptw2['nilai']) - ($bepeg ['bpeg'] + $bbja['bbj'] + $bmoda['bmod']);

						} else if ($data[bulan]=='2'){
							echo number_format(($saldo + $ptw1['nilai'] + $ptw2['nilai'] + $ptw3['nilai'] + $ptw4['nilai']) - ($bepeg2['bpeg'] + $bbja2['bbj'] + $bmoda2['bmod']));
							$kas = ($saldo + $ptw1['nilai'] + $ptw2['nilai'] + $ptw3['nilai'] + $ptw4['nilai']) - ($bepeg2['bpeg'] + $bbja2['bbj'] + $bmoda2['bmod']);
						}
						echo "</b></td>
					</tr>";
				...
				if ($data[bulan] == '1'){
				$sql_bku1 = mysql_query ("SELECT SUM(nilai) AS terima FROM tbl_setor_simpanan WHERE kd_skpd='$kode' AND MONTH(tgl_bukti)<='6'");
				$sql_bku2 = mysql_query ("SELECT SUM(nilai) AS keluar FROM tbl_ambil_simpanan WHERE kd_skpd='$kode' AND MONTH(tgl_bukti)<='6' and jns=0");
				$sql_bku3 = mysql_query ("SELECT SUM(keluar) AS keluar FROM trdrekal WHERE kd_skpd='$kode' AND MONTH(tgl_kas)<='6' AND jns_trans='2'");
				}else{
				$sql_bku1 = mysql_query ("SELECT SUM(nilai) AS terima FROM tbl_setor_simpanan WHERE kd_skpd='$kode' AND MONTH(tgl_bukti)<='12'");
				$sql_bku2 = mysql_query ("SELECT SUM(nilai) AS keluar FROM tbl_ambil_simpanan WHERE kd_skpd='$kode' AND MONTH(tgl_bukti)<='12' and jns=0");
