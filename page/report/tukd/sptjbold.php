...
$bepega2 = mysql_query ("SELECT SUM(a.nilai) AS bpeg FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$skpd' and a.kd_rek5 LIKE '521%'");  	
$bepeg2 = mysql_fetch_array($bepega2);
$bpeg2 = number_format ($bepeg2['bpeg']);

		
$bbaja = mysql_query("SELECT IFNULL(SUM(a.nilai),0) AS bbj FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$skpd' and bulan = '1' and a.kd_rek5 LIKE '522%'");  	
$bbja = mysql_fetch_array($bbaja); 
$bbj = number_format ($bbja['bbj']);

$bbaja2 = mysql_query("SELECT IFNULL(SUM(a.nilai),0) AS bbj FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$skpd' and LEFT(a.kd_rek5,3)='522'");  	
$bbja2 = mysql_fetch_array($bbaja2); 
$bbj2 = number_format ($bbja2['bbj']);	
		
$bmodal = mysql_query ("SELECT IFNULL(SUM(a.nilai),0) AS bmod FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$skpd' and bulan = '1' and a.kd_rek5 LIKE '523%'");	
$bmoda = mysql_fetch_array($bmodal);
$bmod = number_format ($bmoda['bmod']);

$bmodal2 = mysql_query ("SELECT IFNULL(SUM(a.nilai),0) AS bmod FROM trdsptj_belanja a INNER JOIN trhsptj b ON a.no_sptj=b.no_sptj where b.kd_skpd='$skpd' and a.kd_rek5 LIKE '523%'");	
...
$sisasmt2 = number_format(($ptw1['nilai'] + $ptw2['nilai'] + $ptw3['nilai'] + $ptw4['nilai']) - ($bepeg2['bpeg'] + $bbja2['bbj'] + $bmoda2['bmod']));
		...
		echo "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\"> 
				...
				$sql_bku3 = mysql_query ("SELECT SUM(keluar) AS keluar FROM trdrekal WHERE kd_skpd='$skpd' AND MONTH(tgl_kas)<='6' AND jns_trans='2'");
				}else{
				...
				$sql_bku3 = mysql_query ("SELECT SUM(keluar) AS keluar FROM trdrekal WHERE kd_skpd='$skpd' AND MONTH(tgl_kas)<='12' AND jns_trans='2'");
				}	
		...
		echo "<table style=\"border-collapse:collapse;\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\"> 
					 ...
					 <tr>
						<td  align=\"justify\" style=\"font-size:12px;border:solid 0px black;\" colspan=5>Bukti-bukti atas belanja tersebut pada huruf B disimpan pada  $data[nm_skpd] untuk kelengkapan administrasi dan keperluan pemeriksaan sesuai peraturan perundang-undangan. Apabila bukti-bukti tidak benar yang mengakibatkan kerugian daerah, saya bertanggungjawab sepenuhnya atas kerugian daerah dimaksud sesuai kewenangan saya berdasarkan ketentuan perundang-undangan.<br>&nbsp;<br>Demikian surat pernyataan ini dibuat dengan sebenarnya.	</td>
					</tr> ";
	    echo "</table>";          
